...
def serialize_multimodal_data(
    ...
    if not isinstance(multi_modal_data, dict):
        raise TypeError(
            f"multi_modal_data must be a dict or None, got {type(multi_modal_data)}."
        )
    ...
    *,
    ...
    if isinstance(image_data, list):
        ...
        if image_preprocessing_config is None:
            raise RuntimeError(
                "Raw image data require InferenceConfig.image_preprocessing_config."
            )
        return preprocess_image_bytes_list(image_data, image_preprocessing_config)
    if not isinstance(image_data, dict):
        raise TypeError(
